...
if (!fs.existsSync(path.join(__dirname, "data", "db"))) fs.mkdirSync(path.join(__dirname, "data", "db"), { recursive: true });
...
app.post("/api/upload", upload.single("file"), (req, res) => {
  try {
    ...
    if (!file || !clientId) return res.status(400).json({ error: "بيانات ناقصة" });
...
app.get("/api/data", (req, res) => {
  ...
  } catch (err) { res.status(500).json({ error: "خطأ في قراءة البيانات" }); }
});
...
app.post("/api/data", (req, res) => {
  ...
  } catch (err) { res.status(500).json({ error: "خطأ في حفظ البيانات" }); }
});
...
app.post("/api/pdf-data", (req, res) => {
  ...
  if ([".jpg", ".jpeg", ".png"].includes(ext)) contentType = "image/" + ext.replace(".", "");
  ...
  try {
    ...
    if (client) {
      if (type === 'receipts' && client.receipts) {
        client.receipts = client.receipts.filter(f => f.filename !== filename);
      } else if (client.pdfs) {
...
app.get("/api/backup", (req, res) => {
  ...
  res.set({ "Content-Type": "application/zip", "Content-Disposition": `attachment; filename=delta_backup.zip` });
